...
class IsolationForestTester:
    def __init__(self, username, db_uri='mongodb://localhost:27017/', db_name='biometria_db'):
        ...
        self.model = IsolationForest(n_estimators=100, contamination=0.4, random_state=99)
    ...
    def train_model(self):
        ...
        print("\n--- Rozpoczęcie treningu modelu Isolation Forest ---")
        self.model.fit(self.X_train)
        
        y_train_pred = self.model.predict(self.X_train)
        y_train_pred_binary = (y_train_pred == 1).astype(int)
        train_acc = accuracy_score(np.ones(len(y_train_pred)), y_train_pred_binary)
    ...
    def predict(self):
        ...
        self.y_pred = (predictions == 1).astype(int)
        return True
    ...
    def plot_confusion_matrix(self, filename='confusion_matrices_if.png'):
...
if __name__ == '__main__':
    tester = IsolationForestTester('filipp', 'mongodb://localhost:27017/', 'biometria_db')
